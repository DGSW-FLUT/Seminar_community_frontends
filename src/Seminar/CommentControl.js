...
class CommentControl extends Component {
    ...
    DeleteCommentButton(comment) {
        if (comment.commentWriter.username === this.state.me.username) {
            return (
                <span style={{ cursor: 'pointer' }} onClick={() => this.setState(this.deleteComment(comment))}>
                    {' '}
                    삭제하기
                </span>
            );
            ...
            <>
                ...
                {this.state.comments.map((comment) => (
                    <div key={comment.uuid} className="my-4">
                        <div
                            className="px-3 py-2"
                            style={{ display: 'inline-block', borderRadius: '15px', backgroundColor: '#F0F2F5' }}
                        >
                            ...
                            <div dangerouslySetInnerHTML={{ __html: comment.commentContent.replace(/,/gi, '<br/>') }} />
                        </div>
                    </div>
                ))}
            </>
