    ...
    updateSerachResult() {
        ...
        if (keyword.length > 0) {
            axios({
                ...
                this.search_result = this.setState({ search_result: result.data.data.search });
            });
        }
    }

    componentDidUpdate(prv_prop, pr_state, snapshot) {
        if (this.props.location.search !== prv_prop.location.search) this.updateSerachResult();
                ...
                <Container className="p-3">
                    {this.state.search_result.map((post) => (
                        <Card className="m-3 p-3 shadow-sm" style={{ cursor: 'pointer' }} key={post.id}>
                            ...
                            <Card.Text as="h5">
                                {moment(Date.parse(post.createdAt)).fromNow()} - {post.username} -{' '}
                                {this.ViewNamespace(post)}
                            </Card.Text>
                        </Card>
                    ))}
                </Container>
